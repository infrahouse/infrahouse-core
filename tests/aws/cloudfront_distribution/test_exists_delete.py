...
def test_exists_true():
    ...
    mock_client.get_distribution.return_value = {"Distribution": {"Id": DIST_ID, "Status": "Deployed"}}

    with mock.patch.object(CloudFrontDistribution, "_client", new_callable=mock.PropertyMock, return_value=mock_client):
...
def test_delete_already_disabled():
    ...
    with (
        mock.patch.object(CloudFrontDistribution, "_client", new_callable=mock.PropertyMock, return_value=mock_client),
...
def test_wait_until_deployed_timeout():
    ...
    dist = CloudFrontDistribution(DIST_ID, region="us-east-1")
    mock_client = mock.MagicMock()
    mock_client.get_distribution.return_value = {"Distribution": {"Id": DIST_ID, "Status": "InProgress"}}
    ...
    with (
        mock.patch.object(CloudFrontDistribution, "_client", new_callable=mock.PropertyMock, return_value=mock_client),
        mock.patch(_TIMEOUT_PATCH, _noop_timeout),
        mock.patch(_SLEEP_PATCH, side_effect=sleep_raises),
    ):
